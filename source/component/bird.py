...
class Bird():
    def __init__(self, x, y, name):
        ...
        self.pos_timer = 0
        self.path_timer = 0
        self.collide = False  # collided with ground or shape if True
        self.mass = 5.0
    ...
    def check_attack_finish(self):
        if self.pos_timer == 0:
            ...
            distance = tool.distance(self.old_pos[0], self.old_pos[1],
    ...
    def attacking(self, level, mouse_pressed):
        if not self.clicked and mouse_pressed and not self.collide:
            ...
            for sign in bird_list:
                ...
                old = self.phy.body.velocity
                # Scale vertical velocity by c.DIFFICULTY_MULTIPLIER
                vec_y = old[1] * 0.5 * sign * c.DIFFICULTY_MULTIPLIER
                bird.phy.body.velocity = (old[0], vec_y)
                print('bluebird:[', x, ',', y, ']', 'old:', old,
                      '  new:', bird.phy.body.velocity)
...
class YellowBird(Bird):
    ...
    def attacking(self, level, mouse_pressed):
        if not self.clicked and mouse_pressed and not self.collide:
            self.clicked = True
            # Multiply velocity by 3 and also scale by difficulty
            self.phy.body.velocity = self.phy.body.velocity * 3 * c.DIFFICULTY_MULTIPLIER
            print('yellow bird:', self.phy.body.velocity)
...
class BlackBird(Bird):
    ...
    def load_images(self):
        ...
        init_explode_rect_list = [
            (114, 330, 75, 75),
            (481, 330, 75, 75),
            (553, 330, 75, 75),
            (621, 330, 75, 75)
        ]
        self.init_explode_frames = self.load_frames(sheet,
                                                    init_explode_rect_list,
                                                    c.BIRD_MULTIPLIER)
        ...
        self.explode_frames = self.load_frames(sheet, explode_rect_list,
                                               c.BIRD_MULTIPLIER, c.BLACK)

    def attacking(self, level, mouse_pressed):
        if not self.clicked and mouse_pressed and not self.collide:
            self.clicked = True
            self.state = c.EXPLODE
            # Dramatically slow velocity, scaled by difficulty
            self.phy.body.velocity = self.phy.body.velocity * 0.01 * c.DIFFICULTY_MULTIPLIER
        if self.collide:
            # if bird collided with something, it can't explode by clicking
            if not self.clicked:
                self.state = c.INIT_EXPLODE
            self.clicked = True
    ...
    def exploding(self, level):
        if not self.exploded:
            self.change_image(self.explode_frames)
            # Explosion radius (60) scaled by difficulty
            explosion_radius = 60 * c.DIFFICULTY_MULTIPLIER
            level.physics.create_explosion(
                self.phy.body.position,
                self.get_radius(),
                explosion_radius,
                5
            )
...
class WhiteBird(Bird):
    ...
    def attacking(self, level, mouse_pressed):
        if not self.clicked and mouse_pressed and not self.collide:
            self.clicked = True
            vel_x, vel_y = self.phy.body.velocity
            # Scale horizontal velocity by c.DIFFICULTY_MULTIPLIER
            self.phy.body.velocity = (
                vel_x * 2 * c.DIFFICULTY_MULTIPLIER,
                vel_y + 1000
            )
            egg = Egg(self.rect.centerx, self.rect.bottom + 30)
            level.physics.add_egg(egg)
...
class Egg(Bird):
    ...
    def load_images(self):
        sheet = tool.GFX[c.PIG_SHEET]
        frame_rect_list = [(61, 1035, 44, 59)]
        self.frames = self.load_frames(sheet, frame_rect_list,
                                       c.BIRD_MULTIPLIER, c.BLACK)
    ...
    def exploding(self, level):
        if not self.exploded:
            self.change_image(self.explode_frames)
            # Explosion radius (50) scaled by difficulty
            explosion_radius = 50 * c.DIFFICULTY_MULTIPLIER
            level.physics.create_explosion(
                self.phy.body.position,
                self.get_radius(),
                explosion_radius,
