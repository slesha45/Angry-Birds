...
class Pig():
    ...
    def load_frames(self, sheet, frame_rect_list, scale):
        frames = []
        for frame_rect in frame_rect_list:
            frames.append(tool.get_image(sheet, *frame_rect, c.BLACK, scale))
        return frames
    ...
    def setup_images(self):
        """
        A single pig has multiple "states" of appearance:
         - Normal
         - Hurt1
         - Hurt2
        self.images_list is a list of frame lists, one for each visual state.
        """
        self.image_index = 0
        self.image_num = len(self.images_list)
        self.frames = self.images_list[self.image_index]

        # Determine thresholds for switching pig appearance based on its life
        self.image_threshold = []
        for i in reversed(range(self.image_num)):
            i += 1
            temp_life = self.life // self.image_num * i
    ...
    def set_damage(self, damage):
        """
        Reduce pig's life by 'damage' scaled by c.DIFFICULTY_MULTIPLIER.
        Higher DIFFICULTY_MULTIPLIER => more damage => easier to kill pig.
        If you want the opposite (higher = harder), invert the multiplier usage.
        """
        scaled_damage = int(damage * c.DIFFICULTY_MULTIPLIER)
        self.life -= scaled_damage

        # Check if pig needs to change image state
        if self.life < self.image_threshold[self.image_index]:
            self.change_image()
...
class NormalPig(Pig):
    ...
    def load_images(self):
        ...
        normal_rect_list = [(438, 712, 80, 80),
                            (438, 794, 80, 80),
                            (438, 874, 80, 80)]
        hurt1_rect_list = [(522, 794, 80, 80),
                           (522, 872, 80, 80),
                           (522, 948, 80, 80)]
        hurt2_rect_list = [(438, 956, 80, 80),
                           (522, 792, 80, 80),
                           (522, 1026, 80, 80)]
        ...
        for rect_list in rect_lists:
            self.images_list.append(
                self.load_frames(sheet, rect_list, c.NORMAL_PIG_MULTIPLIER)
            )


class BigPig(Pig):
    def __init__(self, x, y):
        # Base life = 16 for big pig
        Pig.__init__(self, x, y, c.BIG_PIG, 16)
    ...
    def load_images(self):
        ...
        for rect_list in rect_lists:
            self.images_list.append(
                self.load_frames(sheet, rect_list, c.BIG_PIG_MULTIPLIER)
            )
